...
def entries : List Entry :=
  [
    { id := Req.EP_CONFORMANCE_AE_REQUIRED
      ...
      leanModule := "StructuralExplainability.EvolutionProtocol"
    ...
    { id := Req.EP_GRAPH_DEFINITION
      ...
      leanModule := "StructuralExplainability.EvolutionProtocol.Graph"
      leanDecl := "GraphDefinition" },

    { id := Req.EP_GRAPH_DELTA
      description := "Structured, non-causal graph change records (deltas)."
      leanModule := "StructuralExplainability.EvolutionProtocol.Graph.Dependency"
      leanDecl := "Delta" },

    { id := Req.EP_GRAPH_EVOLUTION
      description := "Ordered graph histories connecting states via deltas."
      leanModule := "StructuralExplainability.EvolutionProtocol.Graph.ExchangeGraph"
      leanDecl := "Evolution" },
